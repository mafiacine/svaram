...
const App: React.FC = () => {
  ...
  const simulateDownload = useCallback((trackId: string) => {
    ...
    const interval = setInterval(() => {
      progress += Math.floor(Math.random() * 15) + 10; // Faster simulation
      if (progress >= 100) {
        progress = 100;
  ...
  return (
    <div className={`relative h-screen flex flex-col max-w-md mx-auto overflow-hidden transition-all duration-300 shadow-2xl selection:bg-violet-500/30 ${isDarkMode ? 'bg-black text-white' : 'bg-gray-50 text-gray-900'}`}>
      <audio
        ref={audioRef}
      ...
      {!isOnline && (
        <div className={`${isDarkMode ? 'bg-fuchsia-600/90' : 'bg-fuchsia-500'} text-white text-[8px] font-black py-1 flex items-center justify-center gap-2 z-[70] shadow-md backdrop-blur-md uppercase tracking-[0.2em] animate-pulse`}>
      ...
      {isBuffering && (
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-violet-500/10 z-[60] overflow-hidden">
          <div className="h-full bg-violet-500 w-1/4 animate-[loading_1s_infinite_linear] shadow-[0_0_10px_rgba(139,92,246,0.6)]" />
        </div>
      )}
      ...
      <nav className={`h-[68px] border-t flex items-center justify-around px-2 z-[60] shrink-0 pb-1 backdrop-blur-3xl transition-all duration-300 ${isDarkMode ? 'bg-black/90 border-white/5' : 'bg-white/95 border-gray-100'}`}>
        {[
          ...
          <button 
            ...
            className={`group flex flex-col items-center gap-0.5 transition-all duration-200 px-2.5 py-1 rounded-xl relative ${activeTab === tab ? 'text-violet-500 scale-105' : (isDarkMode ? 'text-white/20 hover:text-white/50' : 'text-gray-400 hover:text-violet-500')}`}
          >
            <div className={`w-4.5 h-4.5 transition-all duration-200 transform group-hover:scale-105 ${activeTab === tab ? 'rotate-[-2deg] drop-shadow-[0_0_3px_rgba(139,92,246,0.2)]' : ''}`}>{icon}</div>
            <span className={`text-[7.5px] font-black uppercase tracking-[0.1em] transition-all duration-200 ${activeTab === tab ? 'opacity-100' : 'opacity-40 group-hover:opacity-70'}`}>{label}</span>
          </button>
        ))}
      </nav>
